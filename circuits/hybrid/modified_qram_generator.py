...
def modified_qram(m):
    global cnot_counter, cnot_offset
    gates = ""
    cnot_queue = []
    # STEP 1
    for i in range(m):
        ...
        gates += dual_rail_swap(2 * i, 2 * m)
        # load the root router
        if i == 0:
            # gates += "adding root\n"
            gates += dual_rail_swap(2 * m, middle_qubit(tree_layout[0]))
        else: 
    ...
    # for step 3 later
    return f"{cnot_offset + ((2 ** m) * 2) - 2}\n#\n" + gates + "\n".join(lst)

# with open(f"circuits/modified/size={n}", "w+") as f:
