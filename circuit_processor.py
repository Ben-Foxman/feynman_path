...
with open(f"circuits/modified_shifan/size={n}") as circ:
    # obtain the data for the good simulator
    os.system(f"./fp.out modified_shifan/size={n} -d modified_shifan{n}")
    correct_circuit = circ.readlines()
    # get the data of the correct circuit
    expected = dict()
    with open(f"data/modified_shifan{n}") as res:
        for line in res.readlines():
            ket, real, imag = line.replace("(", "").strip(")\(\n").split(",")
            expected[int(ket)] = float(real) + 1j * float(imag)
    ...
    for x in range(SHOTS):
        ...
        with open(f"circuits/modified_shifan/temp{random_name}.txt", "w") as error_file:
            for gate in error_circuit:
                error_file.write(gate)
        os.system(f"./fp.out modified_shifan/temp{random_name}.txt -d modified_shifan{random_name}error")
        os.remove(f"circuits/modified_shifan/temp{random_name}.txt")

        # get the data of the error circuit
        observed = dict()
        with open(f"data/modified_shifan{random_name}error", "r+") as res:
            for line in res.readlines():
                ket, real, imag = line.replace("(", "").strip(")\(\n").split(",")
                observed[int(ket)] = float(real) + 1j * float(imag)
        os.remove(f"data/modified_shifan{random_name}error")


        # compute the query fidelity for this shot 
        fidelity = qf(2 * n, observed, expected)
        fidelities.append(fidelity)
        print(f"modified_shifan{n}, shot{x}. fidelity=", fidelity)

# write the summary data
with open("output/shifan_circuits.txt", "a+") as f:
    f.write(f"Experiment: modified_shifan{n}, {error_type}. prob = 10^-{exp}. # of monte carlo samples: {SHOTS}. Mean Fidelity: {np.mean(fidelities)}. Std Dev:{np.std(fidelities)}\n\n" )

# write the calculated fidelity for each shot
with open(f"output/shifan_circuit{n}_error={error_type}:10^-{exp}_shots={SHOTS}_output.txt", "w+") as f:
    for fidelity in fidelities:
        f.write(str(fidelity) + "\n")
