...

{
	...
	{
         fscanf(fp, "%s", g);
         if (((!strcmp(g, "\r")))||((!strcmp(g, "\n"))))
                 continue;
         if ((!strcmp(g, "cx"))) h->a[t] = CNOT;
         if ((!strcmp(g, "h"))) h->a[t] = HADAMARD;
         if ((!strcmp(g, "s"))) h->a[t] = PHASE;
         if ((!strcmp(g, "cx"))) h->a[t] = MEASURE;
         fscanf(fp, "%ld", &h->b[t]);
...

{
	...
	printf("\nCHP: Efficient Simulator for Stabilizer Quantum Circuits (Scott Aaronson Implementation)");
	srand(time(0));
	if (argc==1) error(0);
	if (argv[1][0]=='-') param = 1;
	...
	char input[256];
	sprintf(input, "circuits/%s", argv[1]);
	if (param) readprog(h,argv[2],argv[1]);
	else readprog(h,input,NULL);
